...
export default function PayrollPage() {
  ...
  const revenueByEmployeeChart = useMemo(
    () =>
      performance
        .filter((p) => p.revenueGenerated > 0)
        .map((p) => ({ label: p.name, value: p.revenueGenerated })),
    [performance]
  );

  const marginByEmployeeChart = useMemo(
    () =>
      performance
        .filter((p) => p.margin !== null)
        .map((p) => ({
          label: p.name,
          value: p.margin as number,
          color: (p.margin as number) >= 0 ? "var(--moss-lite)" : "var(--oxide)",
        })),
    [performance]
  );
          ...
          {performance.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
              <div className="panel" style={{ padding: 20 }}>
                ...
                <DonutChart data={revenueByEmployeeChart} centerLabel="Total generated" />
              </div>
              <div className="panel" style={{ padding: 20 }}>
                <div className="panel-title" style={{ marginBottom: 14 }}>
                  Margin by employee
                </div>
                <BarChart data={marginByEmployeeChart} emptyLabel="No cost data for this range." />
              </div>
            </div>
          )}
          <div className="panel">
